...
class FreeRoom:
    ...
    def parse_args(self, args):
        for i, item in enumerate(args):
            if item in self.args_rit.keys() and i + 1 < len(args):
                self.args_val_rit[self.args_rit[item]] = args[i + 1]
    ...
    def find_free_rooms_in_time(self, spreadsheet_data, args):
        ...
        if self.args_val_rit['hour'] is not None \
                and self.args_val_rit['hour'] not in self.tools.time_blocks.keys():
            print("Invalid time format")
            return

        if self.args_val_rit['day'] is not None \
                and self.args_val_rit['day'] not in self.tools.days_of_week_label.keys():
            print("Invalid day format")
            return

        d_name = str(self.args_val_rit['day'])
        if self.args_val_rit['day'] is None:
            d_name = '-'

        h_name = str(self.args_val_rit['hour'])
        if self.args_val_rit['hour'] is None:
            h_name = '-'
        ...
        for room_key, room in rooms_details.items():
            is_free = True
            if self.args_val_rit['day'] is not None:
                day = room[self.args_val_rit['day']]
                if day is None:
                    continue

                if self.args_val_rit['hour'] is not None:
                    if day[self.args_val_rit['hour']] is None:
                        print("Free room: ", room_key)
                else:
                    for key, event in room[self.args_val_rit['day']].items():
                        if event is None:
                ...
                for day_key, day_info in room.items():
                    ...
                    if self.args_val_rit['hour'] is not None:
                        if day_info[self.args_val_rit['hour']] is None:
    ...
    def find_free_times_in_room(self, spreadsheet_data, args):
        ...
        if self.args_val_tir['room'] is not None \
                and self.args_val_tir['room'] not in self.tools.time_blocks.keys():
